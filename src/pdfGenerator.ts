...
function drawHeader(
  ...
  doc.image("/src/pdfassets/tpelogo.png", ML, LOGO_Y, {
    width: 240,
    height: 55,
  });
...
function drawProductionHeader(
  ...
  doc.text(b1Label, B1_L + (B1_R - B1_L) / 2 - b1LW / 2, BOX_TOP + 4, {
    lineBreak: false,
  });
  doc.restore();
  ...
  doc.text(
    b2aLabel,
    B2_L + (B2_R - B2_L) / 2 - b2aLW / 2,
    BOX_TOP + (B2_DIV - BOX_TOP) / 2 - 4,
    { lineBreak: false },
  );
  doc.restore();
  ...
  doc.text(
    b2bLabel,
    B2_L + (B2_R - B2_L) / 2 - b2bLW / 2,
    B2_DIV + (BOX_BOT - B2_DIV) / 2 - 5,
    { lineBreak: false },
  );
  doc.restore();
  ...
  doc.text(b3Label, B3_L + (B3_R - B3_L) / 2 - b3LW / 2, BOX_TOP + 4, {
    lineBreak: false,
  });
  doc.restore();
  ...
  const CI = ML + 14;
  t(doc, client.companyName, CI, lY, BOLD, 9, BLACK);
  lY += 12;
  [client.contactName, client.phone, client.email]
    .filter((s) => s && s.trim())
    .forEach((line) => {
      t(doc, line, CI, lY, REG, 9, BLACK);
      lY += 12;
    });
